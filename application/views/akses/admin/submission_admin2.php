...
<?php if($_GET['ss'] == 'kembali' || $_GET['ss'] == 'Kembali'){ ?>
          <div class="box">
            <!-- /.box-header -->
            <div class="box-body">
                ...
                <tr>
                  ...
                  <td>
                    ...
                    <?php if($this->session->userdata('role') == 2){ ?>
                         <?php if(htmlspecialchars($_GET["ss"]) == 'high' ){ ?>
                            <a href="Dashboard/formviewproyek/high/<?php echo $row->id_projek; ?>"><button class="btn btn-primary btn-sm">View</button></a>
                        <?php } else if(htmlspecialchars($_GET["ss"]) == 'medium' ){ ?>
                            <a href="Dashboard/formviewproyek/medium/<?php echo $row->id_projek; ?>"><button class="btn btn-primary btn-sm">View</button></a>
                        <?php } else if(htmlspecialchars($_GET["ss"]) == 'low' ){ ?>
                            <a href="Dashboard/formviewproyek/low/<?php echo $row->id_projek; ?>"><button class="btn btn-primary btn-sm">View</button></a>
                        <?php } else if(htmlspecialchars($_GET["ss"]) == 'kembali' ){ ?>
                            <a href="Dashboard/formviewproyek/kembali/<?php echo $row->id_projek; ?>"><button class="btn btn-primary btn-sm">View</button></a>
                        <?php } else { ?>
                    ...
                    <?php }else if($this->session->userdata('role') == 3 || $this->session->userdata('role') == 4){ ?>
                        ...
                        <a href="javascript:;"><button class="btn btn-primary btn-sm" data-toggle="modal" data-target="#mdl-referensi" onclick="setHdn('<?php echo $row->id_projek; ?>')">Approval User</button></a>
                       <!--  <a href="javascript:;"><button class="btn btn-primary btn-sm" onclick="backAdmin('<?php echo $row->id_projek; ?>')">Back to Admin</button></a> -->
                        <a href="javascript:;"><button class="btn btn-primary btn-sm" data-toggle="modal" data-target="#mdl-backtoadmin" onclick="setHdn('<?php echo $row->id_projek; ?>')">Back to Admin</button></a>
                        <?php }else if($row->status == 8) { ?>
                          <a href="Dashboard/formfinalsadmin/<?php echo $row->id_projek; ?>"><button class="btn btn-primary btn-sm">Review</button></a>
...
<div class="modal fade" id="mdl-backtoadmin" tabindex="-1" role="dialog" aria-labelledby="anomor1" aria-hidden="true">
  <div class="modal-dialog modal-sm" role="document">
    <div class="modal-content">
      <form id="form-back-admin">
        <input type="hidden" name="id_project" class="hdnid_project" value="">
        <div class="modal-body">
          <div class="form">
            <div class="form-group">
              <label>Back to Admin</label>
              <textarea class="form form-control" name="komen" placeholder="YOUR COMENT" required></textarea>
              <!-- <label>*must be filled</label> -->
            </div>
          </div>
...
<script>
  $(function () {
    ...
    //       data: $("#form-back-admin").serialize(), // data yang akan dikirim ke file yang dituju                
    //       success: function(response){ // Ketika proses pengiriman berhasil
    //           $("#mdl-backtoadmin").modal('hide'); // Sembunyikan loadingnya   
    //           location.reload();           
    ...
    $("#sbmt-back-admin").on('click','input, textarea', function(){
      ...
      $.ajax({            
          ...
          url: "<?php echo base_url("index.php/dashboard/backtoadmin"); ?>", // Isi dengan url/path file php yang dituju       
          data: $("#form-back-admin").serialize(), // data yang akan dikirim ke file yang dituju                
          // isi: $("#form-back-admin").required()
          success: function(response){ // Ketika proses pengiriman berhasil              
              $("#form-back-admin").required();
              $("#mdl-backtoadmin").modal('hide'); // Sembunyikan loadingnya   
              location.reload();           
              alert('Back to Admin success')              
          }      
